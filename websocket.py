...
class WebSocketWantReadError(WebSocketError, ssl.SSLWantReadError):
    pass


class WebSocketWantWriteError(WebSocketError, ssl.SSLWantWriteError):
    pass
...
class WebSocket:
    ...
    def accept(self, sock, headers):
        ...
        if self._state == "new":
            self.client = False
            accept = self._accept_headers(headers)
            self.socket = sock
        ...
        raise WebSocketError("WebSocket is in an invalid state")

    def _accept_headers(self, headers):
        if headers.get("upgrade", "").lower() != "websocket":
            raise WebSocketError("Missing or incorrect upgrade header")

        ver = headers.get('Sec-WebSocket-Version')
        if ver is None:
            raise WebSocketError("Missing Sec-WebSocket-Version header")
        ...
        if ver in ['7', '8', '13']:
            ...
            raise WebSocketError("Unsupported protocol version %s" % ver)
        ...
        if protocols:
            ...
            if self.protocol not in protocols:
                raise WebSocketError('Invalid protocol selected')
